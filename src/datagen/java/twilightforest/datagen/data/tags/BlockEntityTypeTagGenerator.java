...
public final class BlockEntityTypeTagGenerator extends FabricTagsProvider.BlockEntityTypeTagsProvider {
	...
	private void addImmovableBlockEntities(TagKey<BlockEntityType<?>> tag) {
		this.builder(tag)
			...
			.add(key(TFBlockEntities.CINDER_FURNACE))
			.add(key(TFBlockEntities.CARMINITE_REACTOR))
			.add(key(TFBlockEntities.REACTOR_DEBRIS))
			.add(key(TFBlockEntities.FLAME_JET))
			.add(key(TFBlockEntities.GHAST_TRAP))
			.add(key(TFBlockEntities.SMOKER))
			.add(key(TFBlockEntities.TOWER_BUILDER))
			.add(key(TFBlockEntities.RED_THREAD))
			.add(key(TFBlockEntities.SINISTER_SPAWNER))
			.add(key(TFBlockEntities.CHISELED_CANOPY_BOOKSHELF))
			.add(key(TFBlockEntities.BRAZIER))
			.add(key(TFBlockEntities.OMINOUS_CANDLE))

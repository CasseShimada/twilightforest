...
public class LichRenderer extends HumanoidMobRenderer<Lich, LichRenderState, LichModel> {
	...
	public LichRenderer(EntityRendererProvider.Context context) {
		...
		this.addLayer(new EyesLayer<>(this) {
			...
			@Override
			public void submit(PoseStack stack, net.minecraft.client.renderer.SubmitNodeCollector nodeCollector, int light, LichRenderState state, float netHeadYaw, float headPitch) {
				if (state.isShadowClone && !state.isInvisible) {
					super.submit(stack, nodeCollector, light, state, netHeadYaw, headPitch);
				}
			}
	...
	protected RenderType getRenderType(LichRenderState state, boolean bodyVisible, boolean translucent, boolean glowing) {
		if (state.isShadowClone && !state.isInvisible) return RenderTypes.entityTranslucent(TEXTURE);
	...
	@Override
	public void submit(LichRenderState state, PoseStack stack, net.minecraft.client.renderer.SubmitNodeCollector nodeCollector, CameraRenderState cameraRenderState) {
		stack.pushPose();

...
public class MagicPaintingRenderer extends EntityRenderer<MagicPainting, MagicPaintingRenderState> {
	...
	@Override
	public void submit(MagicPaintingRenderState state, PoseStack stack, SubmitNodeCollector nodeCollector, CameraRenderState cameraRenderState) {
		...
		if (variant != null) {
			...
			nodeCollector.submitCustomGeometry(stack, RenderTypes.entityTranslucent(textureatlassprite.atlasLocation()), (pose, consumer) -> {
				this.renderPainting(pose, consumer, state.lightCoords, variant.width(), variant.height(), state, manager, textureatlassprite);
			});
			stack.popPose();
		}
	...
	private void renderPainting(PoseStack.Pose pose, VertexConsumer vertex, int[] worldLight, int width, int height, MagicPaintingRenderState state, MagicPaintingTextureManager manager, TextureAtlasSprite backSprite) {

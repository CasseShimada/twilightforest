...
public class ThrownWepRenderer extends EntityRenderer<ThrownWep, ThrownWepRenderState> {
	...
	@Override
	public void submit(ThrownWepRenderState state, PoseStack stack, SubmitNodeCollector nodeCollector, CameraRenderState cameraRenderState) {
		stack.pushPose();
		float spin = state.ageInTicks * 10.0F;
		// size up
		stack.scale(1.25F, 1.25F, 1.25F);
		this.renderDroppedItem(stack, nodeCollector, state, spin);
		stack.popPose();
		super.submit(state, stack, nodeCollector, cameraRenderState);
	}

	private void renderDroppedItem(PoseStack stack, SubmitNodeCollector nodeCollector, ThrownWepRenderState state, float spin) {
		stack.pushPose();
		...
		if (!state.item.isEmpty()) {
			state.item.submit(stack, nodeCollector, state.lightCoords, OverlayTexture.NO_OVERLAY, state.outlineColor);
		}
		stack.popPose();
	}

...
public class NagaSegmentRenderer extends TFPartRenderer<NagaSegment, NagaSegmentRenderState, NagaModel<NagaSegmentRenderState>> {
	...
	@Override
	public void submit(NagaSegmentRenderState state, PoseStack stack, SubmitNodeCollector nodeCollector, CameraRenderState cameraRenderState) {
		if (!state.isInvisible) {
			...
			int previousLight = state.lightCoords;
			state.lightCoords = state.parentLight;
			super.submit(state, stack, nodeCollector, cameraRenderState);
			state.lightCoords = previousLight;
			stack.popPose();
		}
	}

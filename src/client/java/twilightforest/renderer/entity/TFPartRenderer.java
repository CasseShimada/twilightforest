...
public abstract class TFPartRenderer<T extends TFPart<?>, S extends PartEntityState, M extends EntityModel<S>> extends EntityRenderer<T, S> {
	...
	@Override
	public void submit(S state, PoseStack stack, SubmitNodeCollector nodeCollector, CameraRenderState cameraRenderState) {
		stack.pushPose();
		...
		if (rendertype != null) {
			...
			nodeCollector.submitModel(this.model, state, stack, rendertype, state.lightCoords, overlay, k, null, state.outlineColor, null);
		}
	...
	public void extractRenderState(T entity, S state, float partialTick) {
		...
		state.isInWater = entity.isInLiquid();

...
public class BlockChainGoblinRenderer extends HumanoidMobRenderer<BlockChainGoblin, BlockChainGoblinRenderState, BlockChainGoblinModel> {
	...
	@Override
	public void submit(BlockChainGoblinRenderState state, PoseStack stack, SubmitNodeCollector nodeCollector, CameraRenderState cameraRenderState) {
		...
		nodeCollector.submitModel(this.model, null, stack, this.model.renderType(BLOCK_AND_CHAIN_TEXTURE), state.lightCoords, OverlayTexture.NO_OVERLAY, -1, null);
		stack.popPose();
		...
		if (state.deathTime <= 0) {
			...
			BlockChainRenderer.renderChain(xyz, stack, nodeCollector, state.lightCoords, this.chainModel);
			BlockChainRenderer.renderChain(xyz, stack, nodeCollector, state.lightCoords, this.chainModel);
			BlockChainRenderer.renderChain(xyz, stack, nodeCollector, state.lightCoords, this.chainModel);
			BlockChainRenderer.renderChain(xyz, stack, nodeCollector, state.lightCoords, this.chainModel);
			stack.popPose();

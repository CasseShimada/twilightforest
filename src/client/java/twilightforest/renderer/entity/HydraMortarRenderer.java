...
public class HydraMortarRenderer extends EntityRenderer<HydraMortar, HydraMortarRenderState> {
	...
	@Override
	public void submit(HydraMortarRenderState state, PoseStack stack, SubmitNodeCollector nodeCollector, CameraRenderState cameraRenderState) {
		stack.pushPose();
		...
		nodeCollector.submitModel(this.mortarModel, null, stack, this.mortarModel.renderType(TEXTURE), state.lightCoords, OverlayTexture.NO_OVERLAY, -1, null);

		if (state.fuse / 5 % 2 == 0) {
			int color = ARGB.colorFromFloat(alpha, 1.0F, 1.0F, 1.0F);
			nodeCollector.submitModel(this.mortarModel, null, stack, this.mortarModel.renderType(TEXTURE), state.lightCoords, OverlayTexture.pack(OverlayTexture.u(1.0F), 10), color, null);
		}

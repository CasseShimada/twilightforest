...
public class KnightPhantomRenderer extends HumanoidMobRenderer<KnightPhantom, KnightPhantomRenderState, KnightPhantomModel> {
	...
	public KnightPhantomRenderer(EntityRendererProvider.Context context) {
		...
		ArmorModelSet<KnightPhantomModel> armorModels = ArmorModelSet.bake(ModelLayers.PLAYER_ARMOR, context.getModelSet(), KnightPhantomModel::new);
		this.addLayer(new HumanoidArmorLayer<>(this, armorModels, context.getEquipmentRenderer()));
	}

	@Override
	public void submit(KnightPhantomRenderState state, PoseStack stack, net.minecraft.client.renderer.SubmitNodeCollector nodeCollector, CameraRenderState cameraRenderState) {
		if (!state.isDying) {

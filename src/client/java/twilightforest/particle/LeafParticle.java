...
public class LeafParticle extends SingleQuadParticle {
	...
	@Override
	public void extract(QuadParticleRenderState renderState, Camera entity, float partialTicks) {
		...
		this.extractRotatedQuad(renderState, entity, quaternion, partialTicks);
	...
	public record Factory(SpriteSet sprite) implements ParticleProvider<LeafParticleData> {
		...
		@Override
		public Particle createParticle(LeafParticleData data, ClientLevel level, double x, double y, double z, double xSpeed, double ySpeed, double zSpeed, net.minecraft.util.RandomSource random) {
			LeafParticle particle = new LeafParticle(level, x, y, z, xSpeed, ySpeed, zSpeed, 1.0F, this.sprite);

...
public class KnightPhantomModel extends HumanoidModel<KnightPhantomRenderState> implements TrophyBlockModel {
	...
	public void submitTrophy(PoseStack stack, SubmitNodeCollector nodeCollector, int light, int overlay, int color, ItemDisplayContext context, @Nullable ModelFeatureRenderer.CrumblingOverlay breakProgress) {
		if (context == ItemDisplayContext.GUI) {
			...
			nodeCollector.submitModelPart(this.helmet, stack, RenderTypes.entityCutout(PHANTOM_ARMOR_TEXTURE), light, OverlayTexture.NO_OVERLAY, null, ARGB.colorFromFloat(0.0625F, 1.0F, 1.0F, 1.0F), breakProgress, 0);
			stack.popPose();

			stack.scale(1 / 1.1F, 1 / 1.1F, 1 / 1.1F);
			stack.translate(0.0F, 0.25F, 0.0F);
			nodeCollector.submitModelPart(this.head, stack, RenderTypes.entityCutout(KnightPhantomRenderer.TEXTURE), light, overlay, null, color, breakProgress, 0);

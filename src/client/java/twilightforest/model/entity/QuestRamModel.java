...
public class QuestRamModel extends EntityModel<QuestingRamRenderState> implements TrophyBlockModel {
	...
	public void submitColoredSegments(PoseStack stack, SubmitNodeCollector nodeCollector, int light, int overlay, @Nullable ModelFeatureRenderer.CrumblingOverlay breakProgress) {
		for (int i = 0; i < this.segments.length; i++) {
			...
			nodeCollector.submitModelPart(segment, stack, RenderTypes.entityCutout(QuestRamRenderer.TEXTURE), light, overlay, null, dyeRgb, breakProgress, 0);
	...
	public void submitTrophy(PoseStack stack, SubmitNodeCollector nodeCollector, int light, int overlay, int color, ItemDisplayContext context, @Nullable ModelFeatureRenderer.CrumblingOverlay breakProgress) {
		...
		nodeCollector.submitModelPart(this.head, stack, RenderTypes.entityCutout(QuestRamRenderer.TEXTURE), light, overlay, null, color, breakProgress, 0);
		stack.pushPose();
		stack.scale(1.025F, 1.025F, 1.025F);
		nodeCollector.submitModelPart(this.head, stack, RenderTypes.entityTranslucent(QuestRamRenderer.LINE_TEXTURE), 0xF000F0, overlay, null, color, breakProgress, 0);
		stack.popPose();
	}
}

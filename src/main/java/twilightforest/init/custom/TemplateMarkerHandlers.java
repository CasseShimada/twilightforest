...
public final class TemplateMarkerHandlers {
	...
	public static final ResourceKey<TemplateMarkerHandlerList> CAMP_MARKER_HANDLERS = ResourceKey.create(
		TFRegistries.Keys.TEMPLATE_MARKER_HANDLER_LIST,
		TwilightForestMod.prefix("camp_marker_handlers")
	);

	private TemplateMarkerHandlers() {
	}
	...
	private static TemplateMarkerHandlerType register(String name, TemplateMarkerHandlerType type) {
		return Registry.register(TFRegistries.TEMPLATE_MARKER_HANDLER_TYPES, TwilightForestMod.prefix(name), type);
	}
}

...
public class TFPlacedFeatures {
	...
	private static List<PlacementModifier> darkForestTreeCheck(PlacementModifier count) {
		...
		return ImmutableList.of(count, InSquarePlacement.spread(), SurfaceWaterDepthFilter.forMaxDepth(0), PlacementUtils.HEIGHTMAP_NO_LEAVES, new AvoidLandmarkModifier(true, false, 10, HolderSet.empty()), PlacementUtils.filteredByBlockSurvival(TFBlocks.DARKWOOD_SAPLING), BiomeFilter.biome());
	}
	...
	public static void bootstrap(BootstrapContext<PlacedFeature> context) {
		...
		context.register(PLACED_VANILLA_TF_TREES, new PlacedFeature(features.getOrThrow(TFConfiguredFeatures.VANILLA_TF_TREES), tfTreeCheckArea(TFBlocks.TWILIGHT_OAK_SAPLING.defaultBlockState())));

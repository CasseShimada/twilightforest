...
public abstract class CanopyMushroomFeature extends AbstractHugeMushroomFeature {
	...
	protected void placeTrunk(WorldGenLevel levelAccessor, RandomSource random, BlockPos pos, HugeMushroomFeatureConfiguration featureConfiguration, int height, BlockPos.MutableBlockPos mutableBlockPos) {
		...
		for (int b = 0; b < numBranches; b++) {
			bugsLeft = this.buildABranch(levelAccessor, pos, height - 6 + b, this.getLength(random), 0.3 * b + offset, random, new HugeMushroomFeatureConfiguration(featureConfiguration.capProvider(), featureConfiguration.stemProvider(), featureConfiguration.foliageRadius() - 1, net.minecraft.world.level.levelgen.blockpredicates.BlockPredicate.alwaysTrue()), bugsLeft);
		}
	}

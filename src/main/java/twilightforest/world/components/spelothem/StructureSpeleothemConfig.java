...
public record StructureSpeleothemConfig(
	...
	private static Function<RandomSource, Stalactite> compileStalactites(Supplier<SpeleothemVarietyConfig> varietyConfigSupplier) {
		...
		ArrayList<Weighted<Stalactite>> unbakedRandomList = stalactites.stream()
			.map(s -> new Weighted<>(s, Mth.ceil(s.weight() * stoneCounterweight)))
			.collect(Collectors.toCollection(ArrayList::new));
		// Add oreStalactites to the unbaked list
		oreStalactites.stream()
			.map(s -> new Weighted<>(s, Mth.ceil(s.weight() * oreCounterweight)))
			.forEachOrdered(unbakedRandomList::add);

...
public class AvoidLandmarkGridPlacement extends RandomSpreadStructurePlacement {
	...
	public AvoidLandmarkGridPlacement(int spacing, int separation, RandomSpreadType spreadType, int salt) {
		this(spacing, separation, spreadType, salt, Optional.empty());
	}
	...
	public AvoidLandmarkGridPlacement(Vec3i locateOffset, StructurePlacement.FrequencyReductionMethod reduction, float frequencyModifier, int salt, Optional<ExclusionZone> exclusionZone, int spacing, int separation, RandomSpreadType spreadType) {
		this(locateOffset, reduction, frequencyModifier, salt, exclusionZone, spacing, separation, spreadType, Optional.empty());
	}

	public AvoidLandmarkGridPlacement(Vec3i locateOffset, StructurePlacement.FrequencyReductionMethod reduction, float frequencyModifier, int salt, Optional<ExclusionZone> exclusionZone, int spacing, int separation, RandomSpreadType spreadType, Optional<AvoidAdditionalStructures> avoidOtherStructures) {
	...
	@Override
	public boolean applyInteractionsWithOtherStructures(ChunkGeneratorStructureState structureState, int chunkX, int chunkZ) {
		return super.applyInteractionsWithOtherStructures(structureState, chunkX, chunkZ)
			&& (this.avoidOtherStructures.isEmpty() || !this.avoidOtherStructures.get().isPlacementForbidden(structureState, chunkX, chunkZ));
	}
	...
	public record AvoidAdditionalStructures(Object2IntArrayMap<Holder<StructureSet>> avoidStructures) {
		public static final Codec<AvoidAdditionalStructures> CODEC = Codec.unboundedMap(StructureSet.CODEC, Codec.intRange(0, 16))
			.xmap(map -> new AvoidAdditionalStructures(new Object2IntArrayMap<>(map)), AvoidAdditionalStructures::avoidStructures);

		private boolean isPlacementForbidden(ChunkGeneratorStructureState structureState, int chunkX, int chunkZ) {
			for (var entry : this.avoidStructures.object2IntEntrySet()) {
				if (structureState.hasStructureChunkInRange(entry.getKey(), chunkX, chunkZ, entry.getIntValue())) {
					return true;
				}
			}

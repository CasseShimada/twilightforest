...
public record SwitchMarkerHandler(char groupDelimiter, String safeGroupSplitter, char entryDelimiter, String safeEntrySplitter, Map<String, Holder<TemplateMarkerHandler>> handlers) implements TemplateMarkerHandler {
	...
	public SwitchMarkerHandler(char groupDelimiter, char entryDelimiter, Map<String, Holder<TemplateMarkerHandler>> handlers) {
		this(groupDelimiter, Pattern.quote(String.valueOf(groupDelimiter)), entryDelimiter, Pattern.quote(String.valueOf(entryDelimiter)), handlers);
	}

	public SwitchMarkerHandler(Map<String, Holder<TemplateMarkerHandler>> handlers) {
		this('|', ':', handlers);
	}
	...
	public boolean handleDataMarker(String label, BlockPos pos, WorldGenLevel level, RandomSource random, BoundingBox chunkBounds, ChunkGenerator chunkGen, Rotation rotation) {
		...
		for (String group : groups) {
			Holder<TemplateMarkerHandler> handler = this.handlers.get(group);
			if (handler == null) {
				continue;
			}
			String[] entrySplit = group.split(this.safeEntrySplitter, 2);
			String substring = entrySplit.length < 2 ? "" : entrySplit[1];
			if (handler.value().handleDataMarker(substring, pos, level, random, chunkBounds, chunkGen, rotation)) {
				return true;
			}
		}
		return false;

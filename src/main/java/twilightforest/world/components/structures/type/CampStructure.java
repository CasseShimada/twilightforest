...
public class CampStructure extends Structure implements DecorationClearance {
	...
	public Optional<GenerationStub> findGenerationPoint(GenerationContext context) {
		...
		return Optional.of(new GenerationStub(freePosition, structurePiecesBuilder -> {
			TwilightJigsawPiece startingPiece = StructureTemplateDefinitions.INSTANCE.initializeTemplateFromPool(
				CampPieces.START,
				freePosition,
				orientation,
				CampPieces.START.toString(),
				context,
				0,
				false
			);
			...
			startingPiece.addJigsaws(startingPiece, structurePiecesBuilder, context);
		}));
	}

...
public class FortificationShieldAttachment {
	...
	private boolean checkLichCrownBonus(LivingEntity entity) {
		...
		return !entity.getItemBySlot(EquipmentSlot.HEAD).is(TFItems.MYSTIC_CROWN.get()) || (entity.tickCount % 3) != 0;
	}
	...
	public static void addShieldBreakParticles(DamageSource src, LivingEntity entity) {
		...
		if (pos != null) {
			...
			for (int j = 0; j < 16; ++j) {
				...
				particlePacket.queueParticle(TFParticleType.SHIELD_BREAK.get(), pos.x + x, pos.y + y, pos.z + z, x * 0.5D, y * 0.5D, z * 0.5D);
			}
		} else {

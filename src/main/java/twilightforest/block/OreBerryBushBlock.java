...
public class OreBerryBushBlock extends TFBushBlock {
	private static final VoxelShape ENTITY_COLLISION_SHAPE = Block.box(0.001D, 0.0D, 0.001D, 15.999D, 15.999D, 15.999D);
	private final boolean surviveInLight;
	...
	protected void entityInside(BlockState state, Level level, BlockPos pos, Entity entity, InsideBlockEffectApplier effectApplier, boolean isPrecise) {
		if (!(entity instanceof ItemEntity) && level instanceof ServerLevel serverLevel) {
			entity.hurtServer(serverLevel, serverLevel.damageSources().source(TFDamageTypes.OREBERRY), 1.0F);
		}
	}

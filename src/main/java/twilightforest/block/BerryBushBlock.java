...
public class BerryBushBlock extends TFBushBlock implements BonemealableBlock {
	...
	public void randomTick(BlockState state, ServerLevel level, BlockPos pos, RandomSource random) {
		...
		if (random.nextInt(20) == 0 && height < 2 && this.canGrowAt(state, level, pos)) {
	...
	protected void tryGrowUpwards(BlockState state, ServerLevel level, BlockPos pos, RandomSource random) {
		if (random.nextInt(3) == 0 && state.getValue(AGE) >= 2) {
			...
			} else if (above.is(Blocks.SNOW)) {
				level.setBlock(pos.above(), state.getBlock().defaultBlockState().setValue(SNOW_LAYERS, above.getValue(SnowLayerBlock.LAYERS)), Block.UPDATE_CLIENTS);
			}
		}
	}
	...
	public void performBonemeal(ServerLevel level, RandomSource random, BlockPos pos, BlockState state) {
		...
		if (age < 2) {
			this.grow(state, level, pos, Math.min(age + 1 + random.nextInt(2), MAX_AGE - 1));
		}
		this.tryGrowUpwards(state, level, pos, random);
	}
}

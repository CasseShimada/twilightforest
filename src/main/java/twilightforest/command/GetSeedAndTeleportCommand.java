...
public class GetSeedAndTeleportCommand {
	public LiteralArgumentBuilder<CommandSourceStack> register(boolean notIntegratedServer) {
		return Commands.literal("seed_and_tp").requires(stack -> !notIntegratedServer || Commands.LEVEL_GAMEMASTERS.check(stack.permissions())).executes(this::run);
	}
	...
	public static MutableComponent copyOnClickText(MutableComponent component, String text) {
		return ComponentUtils.wrapInSquareBrackets(component.withStyle((style) -> style.withColor(ChatFormatting.GREEN).withClickEvent(new ClickEvent.CopyToClipboard(text)).withHoverEvent(new HoverEvent.ShowText(Component.translatable("chat.copy.click"))).withInsertion(text)));
	}
}
